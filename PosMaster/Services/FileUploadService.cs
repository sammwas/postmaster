...
namespace PosMaster.Services
{
    public class FileUploadService
    {
        ...
        public async Task<FormUploadReturnData> UploadAsync(Guid clientId, IFormFile file)
        {
        ...
        public void Delete(string path)
        {
            if (!string.IsNullOrEmpty(path))
            {
                try
                {
                    var cFile = Path.Combine(_webHostEnvironment.WebRootPath, path);
                    File.Delete(cFile);
                }
                catch (Exception e)
                {
                    Console.WriteLine(e);
                    Console.WriteLine($"Failed to delete file at {path}");
                }
            }
