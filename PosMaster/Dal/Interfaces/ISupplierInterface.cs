...
namespace PosMaster.Dal.Interfaces
{
    public interface ISupplierInterface
    {
        ...
        Task<ReturnData<List<FormSelectViewModel>>> SearchClientSupplierAsync(Guid clientId, string term, int limit = 25);
    }
    ...
    public class SupplierImplementation : ISupplierInterface
    {
        ...
        public async Task<ReturnData<List<FormSelectViewModel>>> SearchClientSupplierAsync(Guid clientId, string term, int limit = 25)
        {
            var result = new ReturnData<List<FormSelectViewModel>> { Data = new List<FormSelectViewModel>() };
            var tag = nameof(SearchClientSupplierAsync);
            _logger.LogInformation($"{tag} get Suppliers for client - {clientId} search -{term}");
            try
            {
                var dataQry = _context.Suppliers
                    .Where(c => c.ClientId.Equals(clientId))
                ...
                var data = await dataQry
                    .Where(c => c.Code.ToLower().Contains(term) ||
                     c.Name.ToLower().Contains(term) || c.Location.Contains(term))
                ...
                if (result.Success)
                    result.Data = data;
                _logger.LogInformation($"{tag} Supplier {clientId} {result.Message} -{data.Count} records");
                return result;
            }
            catch (Exception ex)
            {
